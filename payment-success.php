...
try {
    ...
    page_start('Purchase not found');
    echo '<a href="products.php" class="underline">Browse products</a>';
    page_end();
    exit;
}
$paid = $purchase['status'] === 'paid';
// page_start($paid ? 'Payment successful' : 'Purchase status'); 
?>
...
<head>
    ...
    <title><?= $paid ? 'Payment successful' : 'Purchase status' ?> | LearningBox</title>
...
<main class="flex min-h-[80vh] items-start justify-center px-4 pb-12 pt-28 sm:px-6 sm:pt-32">

<section class="w-full max-w-2xl rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-8">
    <p class="text-lg font-semibold text-brand-navy" id="payment-status"><?= $paid ? 'Thank you! Your payment is confirmed.' : 'Your payment is not confirmed yet.' ?></p>
    <p class="mt-3 text-gray-600"><?= $paid ? 'A receipt is queued for your email. Our team will contact you with the next steps for your purchase.' : 'If you completed payment, please allow a few moments for confirmation. Do not pay again while confirmation is pending.' ?></p>
    <dl class="my-8 grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
        <dt>Product / service</dt>
        <dd><?= esc($purchase['product_name']) ?></dd>
        <dt>Total</dt>
        <dd><?= money($purchase['amount']) ?></dd>
        <dt>Purchase reference</dt>
        <dd>LB-<?= (int)$purchase['id'] ?></dd>
        <dt>Status</dt>
        <dd><?= esc($purchase['status']) ?></dd><?php if ($paid): ?><dt>Payment ID</dt>
            <dd class="break-all"><?= esc($purchase['razorpay_payment_id']) ?></dd>
            <dt>Paid at (UTC)</dt>
            <dd><?= esc($purchase['paid_at']) ?></dd><?php endif; ?>
    </dl>
    <div class="flex flex-wrap gap-3">
      <a class="inline-block rounded-lg bg-primary px-5 py-3 font-bold text-brand-navy" href="products.php">Browse products</a>
      <a href="contact.html" class="inline-block rounded-lg px-2 py-3 underline">Contact support</a>
    </div>
    <?php if (!$paid): ?><button class="block mt-6 underline" onclick="location.reload()">Refresh status</button>
        <script>
            let attempts = 0;
            const timer = setInterval(async () => {
                ...
                try {
                    const r = await fetch('api/payments.php?action=status&token=' + encodeURIComponent(new URLSearchParams(location.search).get('token')));
